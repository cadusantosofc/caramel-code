...
async function fetchNextQuestion(useAutoOffset = false) {
  try {
    // Forçar início do zero se foi resetado (tem prioridade)
    if (sessionStorage.getItem("forceResetModulo") === "true") {
    ...
    // Só adiciona offset se NÃO for forçar reset E tiver offset válido
    if (!useAutoOffset && currentQuestionIdx > 0) {
...
async function timeoutAnswer() {
  ...
  document.getElementById("qSub").innerHTML =
    '<span style="color:#ef4444; font-weight:700;">⏰ TEMPO ESGOTADO!</span>';

  // Oferecer opção de sair após tempo esgotado
  setTimeout(() => {
    if (confirm("Tempo esgotado! Deseja sair e recomeçar a fase depois?")) {
      confirmExit();
    }
  }, 1500);
}
...
function renderQuestion() {
  ...
  if (countEl)
    countEl.textContent = currentQuestionIdx + 1 + "/" + totalQuestions;
  if (fillEl)
    fillEl.style.width =
  ...
  try {
    ...
    if (result.success) {
      ...
      btns.forEach((btn, bIdx) => {
        ...
        // === LÓGICA MELHORADA DE EXPLICAÇÃO ===
        if (
          result.explanations &&
          Array.isArray(result.explanations) &&
          result.explanations[bIdx] !== undefined
        ) {
          expDiv.innerHTML = result.explanations[bIdx]; // Use innerHTML para mostrar os emojis ✅ ❌
        } else if (result.explanation) {
          expDiv.innerHTML = result.explanation;
        } else {
          expDiv.innerText = "Explicação não disponível.";
        }

        expDiv.style.display = "block";

        // Marcação visual
        if (bIdx === result.correct_index) {
          btn.classList.add("correct");
        } else if (bIdx === idx && idx !== -1) {
          btn.classList.add("wrong");
        }
      });

      // Atualiza moedas e XP
      if (result.new_moedas !== undefined) {
...
async function handleEliminate() {
  ...
  if (confirmed) {
    try {
      ...
      if (result.success) {
        ...
        btns.forEach((btn, idx) => {
          if (toHide.includes(idx)) {
            ...
            btn.querySelector(".option-text").style.textDecoration =
              "line-through";
          }
        });
        await db.saveProgress({
      ...
      alert("Erro ao usar varinha mágica.");
    }
  }
}
...
async function confirmExit() {
  const confirmed = await showConfirm(
    "Sair da Fase?",
    "Se você sair agora, todo o progresso desta fase será perdido e você terá que recomeçar do zero.\n\nDeseja realmente sair?",
    "⚠️",
  );
  ...
  try {
    const response = await fetch("api/reset_modulo.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        curso_id: cursoDbId,
        modulo_ordem: phaseId,
      }),
    });
    ...
    if (result.success) {
      ...
      // Força reset na próxima carga
      sessionStorage.setItem("forceResetModulo", "true");
      window.location.href = `course.html?trail=${trailId}`;
    } else {
      alert("Erro ao resetar módulo: " + (result.message || "Desconhecido"));
    }
  } catch (e) {
    console.error(e);
...
async function finishPhase() {
  ...
  await db.saveProgress({
    curso_id: cursoDbId,
    modulo_ordem: phaseId,
    moedas_ganhas: totalPhaseMoedas,
...
function handleBackButton() {
  if (typeof confirmExit === "function") {
    ...
    console.error("confirmExit não encontrada!");
    if (confirm("Deseja realmente sair da fase?")) {
      window.location.href = `course.html?trail=${trailId || "html"}`;
    }
  }
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Limpar qualquer reset pendente antigo
  sessionStorage.removeItem("forceResetModulo");
